...
alter table public.associados
alter column masterclin type text using masterclin::text;
...
declare
  ...
  fk_name text;
begin
  foreach col_name in array array['local_trabalho_id', 'local_pagamento_id', 'escolaridade_id', 'funcao_id']
  loop
    ...
    where conrelid = 'public.associados'::regclass
      and contype = 'f'
      and conkey = array[
        (select attnum from pg_attribute where attrelid = 'public.associados'::regclass and attname = col_name)
      ];
...
begin
  ...
  if target_id is not null then
    ...
    insert into public.associados(
      ativo, matricula, matricula_empresa, nome, cpf, rg, rg_data_emissao, rg_orgao_emissor, rg_uf,

...
begin
  ...
  if target_id is not null then
    ...
    insert into public.associados(
      ...
      data_admissao, data_situacao, data_ficha, endereco, numero, complemento, bairro, cidade, uf, cep, tel1,
      ...
      nullif(upper(trim(payload->>'endereco')), ''), nullif(upper(trim(payload->>'numero')), ''),
      nullif(upper(trim(payload->>'complemento')), ''), nullif(upper(trim(payload->>'bairro')), ''),
      nullif(upper(trim(payload->>'cidade')), ''), nullif(upper(trim(payload->>'uf')), ''),
      nullif(payload->>'cep', ''), nullif(payload->>'tel1', ''), nullif(payload->>'tel2', ''),
      nullif(payload->>'tel3', ''), nullif(lower(trim(payload->>'email')), ''), nullif(payload->>'pis', ''),
      nullif(upper(trim(payload->>'ctps')), ''), nullif(upper(trim(payload->>'ctps_serie')), ''),
